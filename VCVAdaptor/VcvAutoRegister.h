...
//
// Include VcvModule.h rather than this header directly; it pulls in the
// pieces in the order the templates need.
//
// DEFAULTS come from macros so a plugin sets them once (in CMake) instead of
// once per module:
//
//     VCV_MODULE_ID_PREFIX   "VCV: "          plugin id is prefix + slug
//     VCV_MODULE_CATEGORY    "VCV"
//     VCV_MODULE_VENDOR      "VCV (ported)"
//
// OPT OUT per module by defining VCV_NO_AUTO_REGISTER before including its
// .cpp, then calling vcv::registerModule() yourself with whatever options you
// need. Opt out of the editor for the whole plugin with VCVADAPTOR_NO_GUI.
...
#ifndef VCV_MODULE_ID_PREFIX
	#define VCV_MODULE_ID_PREFIX "VCV: "
#endif

#ifndef VCV_MODULE_CATEGORY
	#define VCV_MODULE_CATEGORY "VCV"
#endif

#ifndef VCV_MODULE_VENDOR
	#define VCV_MODULE_VENDOR "VCV (ported)"
#endif
...
void autoRegisterModel(rack::Model& model)
{
	...
	slug = model.slug;
	id   = std::string(VCV_MODULE_ID_PREFIX) + model.slug;
	...
	opt.id       = id.c_str();
	opt.name     = slug.c_str();
	opt.category = VCV_MODULE_CATEGORY;
	opt.vendor   = VCV_MODULE_VENDOR;
